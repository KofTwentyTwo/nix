...
**Never use `~/.Codex/session-state.md`** - always use local `./docs/` files for state.
...
2. Integrate each into the right home:
   ...
   - Project-specific facts about this repo → this `AGENTS.md`
   - Per-project memory → `~/.Codex/projects/<sanitized-cwd>/memory/`
   - Permissions / hooks / MCP servers → `~/.Codex/settings.json` or `home/Codex/default.nix`
...
| `Codex/` | MCP servers, permissions, settings, plugins |
