...
- **Secrets**:
  ...
  - **Always-on GitHub auth (since 2026-07-14):** `secrets/github-token.enc` (classic PAT, no expiry; master copy = 1Password item `GITHUB_TOKEN` in `NixEnvironmentVariables`) deploys to `~/.config/secrets/github-token` and is exported as `GITHUB_TOKEN` in every interactive shell (`home/zsh`) — so `gh` never needs `gh auth login`, and HTTPS git remotes auth headlessly via the `!gh auth git-credential` helper in `flake.nix`'s `programs.git`. SSH remotes ride the 1Password agent (`home/ssh`). Rotate: update the 1Password item, `sops secrets/github-token.enc`, rebuild.
