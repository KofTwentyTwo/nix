...
- **Secrets**:
  ...
  - **Sandboxed-consumer exception** (e.g. Claude Cowork scheduled tasks): the sandbox only mounts specific paths (the project folder + `~/Git.Local/dmd`); sops-nix's default symlink-into-`~/.config/sops-nix/` dangles inside the sandbox. For these, bypass sops-nix's `secrets.*` block and use a `home.activation` script that decrypts directly to a regular file at the consumer's mount point. See `home/sops/default.nix` `deployGithubSecurityPat` for the canonical pattern.
